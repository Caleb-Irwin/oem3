...
export const files = pgTable(
	'files',
	{
		...
		author: varchar('author', { length: 256 }).references(() => users.username, {
			onDelete: 'no action'
		}),
...
export const images = pgTable(
	...
	(images) => [
		index('images_sourceURL_idx').on(images.sourceURL),
		index('images_productId_idx').on(images.productId)
	]
);
