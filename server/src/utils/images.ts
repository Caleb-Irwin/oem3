...
export async function uploadImageAndThumb(conf: {
	filePath: string;
	...
	if (
		conf.filePath.toLowerCase().endsWith('.jpg') ||
		conf.filePath.toLowerCase().endsWith('.jpeg')
	) {
	...
	await uploadImage({
		...conf,
		content: thumbContent as Buffer,
		filePath: `/thumbnail/${conf.filePath}`,
		isThumb: true
	});
}
