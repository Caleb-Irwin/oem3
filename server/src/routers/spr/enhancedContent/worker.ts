...
work({
	process: async ({ db, progress }) => {
		...
		//images;
		const imageData = readCSV<[string, string, string]>('/basic/EN_CA_B_productimages.csv').map(
			(row): typeof sprImages.$inferInsert => {
				return {
					etilizeId: row[0],
					type: row[1],
					status: row[2] as
		...
		await db.transaction(async (db) => {
			const missingEtilizeItems = await db
				...
				.from(sprFlatFile)
				.leftJoin(sprEnhancedContent, eq(sprFlatFile.etilizeId, sprEnhancedContent.etilizeId))
			...
			for (const row of enhancedContentRows) {
				const primaryImages = row.images
						.filter((v) => ['Large', 'Original'].includes(v.type) || !isNaN(parseInt(v.type)))
						.map((v) => v.type),
					otherImages = row.images
						.filter((v) => !primaryImages.includes(v.type) && !['Thumbnail'].includes(v.type))
						.map((v) => v.type),
					orderedOtherImages = [
				...
				if (
					...
					await db
						...
						.where(eq(sprFlatFile.etilizeId, row.etilizeId))
						.execute();
				}
			}
			...
			const nowDeletedEnhancedItems = await db
				...
				.leftJoin(sprFlatFile, eq(sprEnhancedContent.etilizeId, sprFlatFile.etilizeId))
