  ...
  process: async ({
    ...
    await db.transaction(async (db) => {
      const prevItems = new Map(
        (await db.query.sprPriceFile.findMany({ with: { uniref: true } })).map(
...
function transformPriceFile(
  ...
  return {
    ...
    netPriceCents: removeNaN(
      Math.round(parseFloat(item["Net Price"] as string) * 100)
    ) as number,
    listPriceCents: removeNaN(
      Math.round(parseFloat(item["List Price"] as string) * 100)
    ) as number,
    lastUpdated: 0,
  };
}
...
function getStatus(
  status: string
): "Active" | "Discontinued" | "Not available" | null {
