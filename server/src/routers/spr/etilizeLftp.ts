...
export function getEtilizeCredentials(): EtilizeCredentials {
	...
	if (!user || !password) {
		throw new Error('ETILIZE_USER and ETILIZE_PASSWORD must be set for Novexco Etilize downloads');
	}
...
async function runLftp(
	...
	try {
		...
		if (/executable not found|enoent/i.test(detail)) {
			throw new Error(
				'lftp is required for Novexco Etilize downloads but was not found in PATH. ' +
					'Install it with `brew install lftp` on macOS or `sudo apt-get install lftp` on Debian/Ubuntu.'
			);
		}
		throw new Error(`Unable to start lftp for Novexco Etilize downloads: ${detail}`);
	}
	...
	if (timedOut) {
		throw new Error('lftp timed out during a Novexco Etilize download');
	}

	if (exitCode !== 0) {
		const safeError = redactSecrets(errorOutput.trim(), credentials).slice(0, 4_000);
		throw new Error(
			`lftp failed during a Novexco Etilize download (exit ${exitCode})${safeError ? `: ${safeError}` : ''}`
		);
	}
}
