...
import { db } from '../db';
import { type Context, generalProcedure, router, viewerProcedure } from '../trpc';
import { z } from 'zod';
...
export const labelsRouter = router({
	sheet: {
		all: viewerProcedure.query(async ({ ctx }) => {
			...
			return allSheets.filter(
				(sheet) =>
					ctx.user.permissionLevel === 'admin' || sheet.public || sheet.owner === ctx.user.username
			);
		}),
		...
		rename: generalProcedure
			...
			.mutation(async ({ ctx, input: { id, name, isPublic } }) => {
				await checkSheetPermissions(ctx, id);
				await db.update(labelSheets).set({ name, public: isPublic }).where(eq(labelSheets.id, id));
				update();
			})
	},
	allLastAccessed: viewerProcedure.query(async ({ ctx }) => {
	...
	allSub: createSub<{ sheetId: number }, (typeof labels.$inferSelect)[]>(
		async ({ ctx, input: { sheetId } }) => {
	...
	add: generalProcedure
		...
		.mutation(async ({ ctx, input: { sheetId, name, price, barcode, qbId } }) => {
			await checkSheetPermissions(ctx, sheetId);
	...
	edit: generalProcedure
		...
		.mutation(async ({ ctx, input: { sheetId, id, name, price, barcode, qbId } }) => {
			await checkSheetPermissions(ctx, sheetId);
			await db
				.update(labels)
				.set({
...
const checkSheetPermissions = async (ctx: Context, id: number): Promise<void> => {
	...
	if (ctx.user.permissionLevel === 'admin' || sheet.public || sheet.owner === ctx.user.username)
		return;
