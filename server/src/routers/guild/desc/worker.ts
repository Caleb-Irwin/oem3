...
work({
	process: async ({ db, progress }) => {
		...
		console.log(`Description scrape finished - ${fail} failed, ${success} succeeded`);
		...
		async function addDescription(
			...
			const prev = await db.query.guildDescriptions
				.findFirst({
					where: (guildDescriptions, { eq }) => eq(guildDescriptions.gid, guildId)
				})
				.execute();
			...
			try {
				...
				const { name, itemPrice, uom, mediaDtos, longDescription: longDescHtml } = raw.items[0];
				...
				await db
					...
					.where(eq(guildData.gid, guildId))
					.execute();
				success++;
			} catch (e) {
				console.log(e);
