...
async function getSession(
	...
	if (originalSession && mode !== 'refresh') return { contents: originalSession };
	...
	const setCookies = setCookieParser(res.headers.getSetCookie(), {
		...
		rememberMe = setCookies['REMEMBERME']?.value ?? originalSession?.vRememberMe,
...
export class SessionManager {
	...
	async init(previousSession?: Session, mode: 'default' | 'refresh' | 'new' = 'default') {
		this.initing = true;
		const sessionFull = await getSession(previousSession, mode);
		if (sessionFull.contents === undefined) throw new Error('Failed to create session');
