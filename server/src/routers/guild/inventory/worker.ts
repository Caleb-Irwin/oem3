...
work({
	process: async ({ db, message, progress, utils: { getFileDataUrl, createChangeset } }) => {
		const fileId = (message as { fileId: number }).fileId,
			...
			res = Papa.parse(atob(dataUrl.slice(dataUrl.indexOf('base64,') + 7)).replace('ï»¿', ''), {
				header: true
			});

		await db.transaction(async (db) => {
			const prevItems = new Map(
				(await db.query.guildInventory.findMany({ with: { uniref: true } })).map((item) => [
					item.gid,
					item
				])
			);
			...
			await changeset.process({
				db,
				rawItems: (res.data as GuildInventoryRaw[]).filter((item) => (item.SKU.length ?? '') > 0),
				prevItems,
				transform: transformGuildInventory,
				extractId: (item) => item.gid,
				diff: genDiffer(
					['onHand'],
					['gid', 'onHand', 'sku', 'upc', 'spr', 'basics', 'cis', 'um', 'qtyPerUm']
				),
				progress,
				fileId
			});
...
function transformGuildInventory(item: GuildInventoryRaw): typeof guildInventory.$inferInsert {
	return {
