	...
	process: async ({
		...
		await db.transaction(async (db) => {
			const prevItems = new Map(
				(await db.query.guildData.findMany({ with: { uniref: true } })).map((item) => [
					item.gid,
					item
				])
			);
...
const transformGuild = (g: GuildRaw): typeof guildData.$inferInsert => {
	return {
		...
		cis: (g['CIS#'] ?? '').trim(),
		priceL0Cents: removeNaN(Math.round(parseFloat(g['Price Level 0'] as string) * 100 || -1)) ?? -1,
		priceL1Cents: removeNaN(Math.round(parseFloat(g['Price Level 1'] as string) * 100 || -1)) ?? -1,
		priceRetailCents:
			removeNaN(Math.round(parseFloat(g['Retail Price Level'] as string) * 100 || -1)) ?? -1,
		memberPriceCents:
			removeNaN(Math.round(parseFloat(g['Member Price'] as string) * 100 || -1)) ?? -1,
		dropshipPriceCents:
			removeNaN(Math.round(parseFloat(g['Dropship Price'] as string) * 100 || -1)) ?? -1,
		...
		webCategory2Desc: convertZeroToEmpty(g["Web Category 2 'Sub' Descriptions"]),
		webCategory3Desc: convertZeroToEmpty(g["Web Category 3 'Sub-Sub' Descriptions"]),
		webCategory4Desc: convertZeroToEmpty(g["Web Category 4 'Sub-Sub-Sub' Descriptions"]),
		um: enforceEnum((g['ENglish Unit'] ?? '').toLowerCase(), guildUmEnum.enumValues),
		standardPackQty: removeNaN(parseInt(g['Standard Pack Qty'] as string)) ?? -1,
		masterPackQty: removeNaN(parseInt(g['Master Pack Qty'] as string)) ?? -1,
		freightFlag: (g['Freight Flag']?.trim() ?? '') === 'F',
		weightGrams: removeNaN(Math.ceil(parseFloat(g['Shipping Weight'] as string) * 453.592)) || -1,
		heavyGoodsChargeSkCents:
			removeNaN(Math.round(parseFloat(g['Heavy Goods Chg_SK'] as string) * 100)) ?? 0,
		minOrderQty: parseInt(g['Min. Qty Order'] as string) || -1,
		guildDateChanged: new Date(Date.UTC(0, 0, parseInt(g['Date Changed'] as string) - 1)).valueOf(),
		lastUpdated: 0
	};
};
