...
work({
	process: async ({ db }) => {
		...
		async function updateSearchIndex<T extends ChangesetTable>(
			...
			const resourceName = getTableConfig(searchTable).name as Exclude<ResourceType, 'changeset'>,
				lastSearchUpdate = parseInt((await kv.get('lastSearchUpdate/' + resourceName)) ?? '0');
			...
			newLastSearchUpdate = String(toUpdate[0]?.lastUpdated ?? lastSearchUpdate);
			await kv.set('lastSearchUpdate/' + resourceName, newLastSearchUpdate as string);
		}

		await updateSearchIndex(qb, (item) => {
			const baseId = item.qbId.includes(':') ? item.qbId.split(':')[1] : item.qbId;
			return {
				keyInfo: `${baseId.includes(' ') || baseId.includes('-') ? '' : baseId}`,
				otherInfo: `${item.desc} ${
					!baseId.includes(' ') && baseId.length < 20 ? getSubStrings(baseId) : baseId
				}`
			};
		});
