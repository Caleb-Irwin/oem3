export function validateCustomValue(
	...
	cellType: 'string' | 'number' | 'boolean',
	nullable: boolean
): { res: string | null | undefined; error: string | null } {
	if (!value) value = '';
	...
	switch (cellType) {
		...
		case 'number': {
			...
			if (isNaN(numValue) || !isFinite(numValue)) {
				return {
					res: undefined,
					error: 'Value must be a valid number' + (nullable ? ' or Null' : '')
				};
			}
			if (Math.round(numValue) !== numValue) {
				return {
					res: undefined,
					error: 'Value must be an integer (whole number)' + (nullable ? ' or Null' : '')
				};
			}
			return { res: value, error: null };
		}
		...
		case 'boolean': {
			...
			if (lowerValue === 'true') {
				...
				return {
					res: undefined,
					error: 'Value must be true or false' + (nullable ? ' or Null' : '')
				};
			}
		}
		default:
...
export function objectsEqual<T extends object, U extends object>(
	obj1: T | U | null | undefined,
