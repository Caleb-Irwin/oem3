<script lang="ts">
	...
	let { data, canEdit, onAssign }: Props = $props();
	let selectedOrderId: number | null = $state(null);
	let requestedOrderId: number | null = $state(null);
	...
	// Finished orders are paged in on demand; years of them must never all be rendered.
	...
	async function loadHistory(reset = false) {
		...
		try {
			const offset = reset ? 0 : completedOrders.length;
			...
			if (requestId === historyRequest) loadingHistory = false;
		}
	}
	...
	const availableItems = $derived.by(() => {
		...
		const qbRowsInSelectedOrder = new Set(
		...
		for (const item of data.items) {
			if (item.orderStatus === 'completed') continue;
			if (qbRowsInSelectedOrder.has(item.qbRow)) continue;
	...
	const selectedDuplicateCount = $derived(
		selectedItems.filter((item) => ordersForItem(item, selectedOrder?.id).length > 0).length
	);

	$effect(() => {
		if (requestedOrderId !== null) {
			if (data.orders.some((order) => order.id === requestedOrderId)) {
			...
			!data.orders.some((order) => order.id === selectedOrderId) &&
			!completedOrders.some((order) => order.id === selectedOrderId)
		) {
			selectedOrderId =
				data.orders.find((order) => order.status === 'draft')?.id ??
				data.orders.find((order) => order.status === 'sent')?.id ??
				data.orders[0].id;
		}
	});
	...
	function statusLabel(status: 'draft' | 'sent' | 'completed') {
		return status === 'draft' ? 'Open' : status === 'sent' ? 'Sent' : 'Completed';
	}

	function statusClass(status: 'draft' | 'sent' | 'completed') {
		return status === 'draft'
	...
	function ordersForItem(item: OrderPlannerItemData, excludedOrderId?: number) {
	...
	function toggleSupplier(items: OrderPlannerItemData[]) {
