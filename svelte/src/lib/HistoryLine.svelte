...
<li class="flex items-stretch py-0.5">
	...
	<div
		class="flex items-center min-w-20 lg:min-w-40 px-2 border-l-8 {entry.entryType === 'create'
			? 'border-primary-500'
	...
	<div class="flex flex-wrap gap-2">
		{#if updateData}
			{#each updateData as change}
				<span class="chip hover:cursor-default">{change[0]}: {change[1]} -> {change[2]}</span>
			{/each}
		{/if}
		{#if createData}
			{#each Object.entries(createData) as [key, value]}
				<span class="chip hover:cursor-default">{key}: {value}</span>
			{/each}
		{/if}
	</div>
</li>
